...
export async function generateMetadata({
  ...
  if (!info?.languageSchool) {
    return { title: "Sayfa Bulunamadı | Aka Eğitim" };
  }

  return {
    title: `${info.languageSchool.title} | Aka Eğitim`,
    description: info.languageSchool.description,
    keywords: [
      `${info.name.toLowerCase()} dil okulu`,
      `${info.name.toLowerCase()} dil eğitimi`,
      "yurtdışı dil okulu",
      "aka eğitim",
    ],
    alternates: {
      canonical: `/ulkeler/${country.toLowerCase()}/dil-okulu`,
    },
  };
}
...
export default async function LanguageSchoolPage({ params }: PageProps) {
  const { country } = await params;
  const info = getCountryBySlug(country.toLowerCase());
  ...
  return (
    <main className="bg-white">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 py-16 w-full">
          <div className="max-w-3xl">
            <nav className="text-white/80 mb-4 text-sm">
              ...
              <span className="text-white">Dil Okulu</span>
            </nav>
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-white mb-6 leading-tight">
              {languageSchool.title}
            </h1>
      ...
      <section className="py-16 sm:py-20 bg-white">
        <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
          <div className="text-center mb-12">
            ...
            >
              Neden {info.name} Dil Eğitimi?
            </h2>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {languageSchool.whyChooseThis.map((reason, index) => (
              <div
                key={index}
      ...
      >
        <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {languageSchool.programs.map((program, index) => (
              <div
                key={index}
          ...
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            <div>
              ...
              <div className="space-y-4">
                {languageSchool.requirements.map((req, index) => (
            ...
            <div>
              ...
              <div className="space-y-4">
                {languageSchool.process.map((step, index) => (
                  <div key={index} className="flex items-start">
                    <div
                      className="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center mr-3 mt-0.5"
                      style={{
                        background:
                          index === languageSchool.process.length - 1
                            ? "var(--primary)"
                            : "var(--primary-light)",
                      }}
                    >
      ...
      {/* FAQs Section */}
      {languageSchool.faqs && languageSchool.faqs.length > 0 && (
        <section
          className="py-16 sm:py-20"
          style={{
            background:
        ...
        >
          <div className="max-w-4xl mx-auto px-6 sm:px-8 lg:px-12">
            ...
            <div className="space-y-6">
              {languageSchool.faqs.map((faq, index) => (
                <div
                  key={index}
      ...
      <section className="py-16 sm:py-20 bg-gradient-to-br from-gray-50 to-white">
        <div className="max-w-4xl mx-auto px-6 sm:px-8 lg:px-12 text-center">
          <h2
            ...
            style={{ color: "var(--primary-dark)" }}
          >
            {info.name} Dil Eğitimine Başlamaya Hazır Mısınız?
          </h2>

...
export function getCityRepresentatives(
  cityName: string
): CityRepresentatives | null {
  return cityRepresentativesData[cityName] || null;
}

// Tüm temsilcileri getir
export function getAllRepresentatives(): Representative[] {
  return Object.values(cityRepresentativesData).flatMap(
    (city) => city.representatives
  );
}
...
export const countryRepresentativesData: Record<
  string,
  Record<string, CityRepresentatives>
> = {
  İtalya: {
    Lombardy: {
...
export function getCountryMapConfig(
  countryName: string
): CountryMapConfig | null {
